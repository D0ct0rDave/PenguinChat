...
BOOL Comm_RecvMessage(unsigned int Recipient,TMessage *Msg)
{
    ...
    switch(Recipient)
    {
        ...
        Res = ReadFile(CommonServHandle,Msg->MsgData,MAX_MSG_LEN,&BytesLeidos,(LPOVERLAPPED) NULL);
        break;

        case MSG_RECIPIENT_PRIVATE_PIPE:
        Res = ReadFile(PrivateServHandle,Msg->MsgData,MAX_MSG_LEN,&BytesLeidos,(LPOVERLAPPED) NULL);
        break;
    }
